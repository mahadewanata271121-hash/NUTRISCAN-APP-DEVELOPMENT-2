...
class AnalysisHistoryActivity : AppCompatActivity() {
    ...
    private fun setupBarChart(sharedStats: android.content.SharedPreferences, consumedCal: Float, dailyGoal: Float) {
        ...
        for (i in graphDays.indices) {
            val dayKey = dayMapping[graphDays[i]] ?: ""
            val value = when {
                graphDays[i] == dayOfWeek -> consumedCal
                isPastDay(graphDays[i], dayOfWeek) -> sharedStats.getFloat("history_cal_$dayKey", 0f)
                else -> -1f
            }
            if (value >= 0) {
    ...
    private fun setupPaginationButtons() {
        listOf(R.id.btn_next_cal, R.id.btn_next_carbs, R.id.btn_next_prot, R.id.btn_next_fat).forEach { id ->
    ...
    private fun updatePaginationVisibility(interval: String) {
        val visibility = if (interval == "6B") View.VISIBLE else View.GONE
        val rotation = if (currentMonthOffset == 0) 180f else 0f
        listOf(R.id.btn_next_cal, R.id.btn_next_carbs, R.id.btn_next_prot, R.id.btn_next_fat).forEach { id ->
            findViewById<ImageButton>(id)?.apply { this.visibility = visibility; this.rotation = rotation }
        }
    }
    ...
    private fun updateCharts(interval: String) {
        ...
        val calPts = mutableListOf<Float>()
        val carbsPts = mutableListOf<Float>()
        val protPts = mutableListOf<Float>()
        val fatPts = mutableListOf<Float>()
        ...
        when (interval) {
            "1M" -> {
                ...
                val graphDays = listOf(Calendar.MONDAY, Calendar.TUESDAY, Calendar.WEDNESDAY, Calendar.THURSDAY, Calendar.FRIDAY, Calendar.SATURDAY, Calendar.SUNDAY)
                for (i in graphDays.indices) {
                    if (graphDays[i] == today) {
                        calPts.add(stats.getFloat("consumed_calories", 0f))
                        carbsPts.add(stats.getFloat("consumed_carbs", 0f))
            ...
            "1B" -> {
                ...
                val currentWeekInMonth = (calendar.get(Calendar.DAY_OF_MONTH) - 1) / 7
                for (i in 0..3) {
                    if (i == currentWeekInMonth) {
                        calPts.add(stats.getFloat("consumed_calories", 0f))
                        carbsPts.add(stats.getFloat("consumed_carbs", 0f))
                        protPts.add(stats.getFloat("consumed_protein", 0f))
                        fatPts.add(stats.getFloat("consumed_fat", 0f))
            ...
            "6B" -> {
                val p1 = listOf("Jan", "Feb", "Mar", "Apr", "Mei", "Jun")
                val p2 = listOf("Jul", "Agu", "Sep", "Okt", "Nov", "Des")
                labels = if (currentMonthOffset == 0) p1 else p2
                val currentMonth = calendar.get(Calendar.MONTH)
                
                for (i in 0..5) {
                    val actualMonth = if (currentMonthOffset == 0) i else i + 6
    ...
    private fun isPastDay(graphDay: Int, today: Int): Boolean {
        val order = mapOf(Calendar.MONDAY to 0, Calendar.TUESDAY to 1, Calendar.WEDNESDAY to 2, 
    ...
    private fun formatLimit(v: Float): String = if (v >= 10000) String.format(Locale.US, "%.1fk", v / 1000) else v.toInt().toString()
}
